...
//Write a derived query to list all tickets between a range of dates
    List<Ticket> findAllByDateTimeBetween(LocalDateTime localDate1, LocalDateTime localDate2);
// ------------------- JPQL QUERIES ------------------- //
//Write a JPQL query that returns all tickets are bought from a specific user
    @Query("SELECT t FROM Ticket t WHERE t.user.id= ?1")
    List<Ticket> retrieveAllTicketsBySpecificUser(Long userId);
//Write a JPQL query that returns all tickets between a range of dates
    @Query("SELECT t FROM Ticket t WHERE t.dateTime BETWEEN ?1 AND ?2")
    List<Ticket> retrieveAllTicketsByBetweenDates(LocalDateTime localDateTime1,LocalDateTime localDateTime2);
// ------------------- Native QUERIES ------------------- //
//Write a native query to count the number of tickets a user bought
    @Query(value = "SELECT COUNT (*) FROM Ticket WHERE user_account_id=?1",nativeQuery = true)
    Integer countAllSoldTickets(Long userId);
