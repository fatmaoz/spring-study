...
//Write a JPQL query to sort all accounts with age
    ...
    List<Account> retrieveallAccountsSorting();
// ------------------- Native QUERIES ------------------- //
//Write a native query to read all accounts with an age lower than a specific value
    @Query(value = "SELECT * FROM account_details a WHERE a.age<?1", nativeQuery = true)
    List<Account> retrieveaccountsLowerThan(Integer age);
//Write a native query to read all accounts that a specific value can be containable in the name, address, country, state city
@Query(value = "SELECT * FROM account_details WHERE name ILIKE concat('%',?1,'%') OR " +
                "country ILIKE concat('%',?1,'%') OR address ILIKE concat('%',?1,'%') " +
                "OR state ILIKE concat('%',?1,'%')",nativeQuery = true)

List<Account> retrieveSpesificValue(String pattern);
//Write a native query to read all accounts with an age lower than a specific value
