...
//Write a JPQL query to list all movies between a range of prices
    ...
    List<Movie> retrievePricesBetween(BigDecimal price1, BigDecimal price2);
...
//Write a native query that returns a movie by name
    ...
    Optional<Movie> retrieveAllUniqeMovieNames();
//Write a native query that return the list of movies in a specific range of prices
    @Query(value = "SELECT * FROM movie m WHERE m BETWEEN ?1 AND ?2",nativeQuery = true)
    List<Movie> retrieveAllPricesBetweenSpesificRange(BigDecimal price1, BigDecimal price2);
//Write a native query to return all movies where duration exists in the range of duration
    @Query(value = "SELECT * FROM movie m WHERE duration IN ?1",nativeQuery = true)
    List<Movie> retrieveMoviesByDurationExist(List<Integer> durations);
